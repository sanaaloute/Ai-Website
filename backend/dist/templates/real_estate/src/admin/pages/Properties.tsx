...
const emptyProperties: Partial<Properties> = {
  name: '',
  slug: '',
  location: '',
  price: 0,
...
export default function AdminProperties() {
  ...
  const [editing, setEditing] = useState<Properties | null>(null);
  const [form, setForm] = useState<Partial<Properties>>(emptyProperties);
  const [saving, setSaving] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<Properties | null>(null);
  ...
  const load = async (targetPage = page) => {
    ...
    try {
      const [p, c] = await Promise.all([
        listAllProperties(targetPage, PER_PAGE, {
  ...
  const openCreate = () => {
    setEditing(null);
    setForm(emptyProperties);
    setModalOpen(true);
  };

  const openEdit = (item: Properties) => {
    setEditing(item);
    setForm({ ...item });
    setModalOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const data = { ...form };
      if (editing) {
        await updateProperties(editing.id, data);
      } else {
      ...
      addToast({
        variant: 'error',
        title: 'Save failed',
        message: err instanceof Error ? err.message : 'Could not save property',
      });
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await deleteProperties(deleteTarget.id);
      setDeleteTarget(null);
      await load();
      addToast({ variant: 'success', title: 'Deleted', message: 'Property has been removed.' });
    } catch (err) {
      addToast({
        variant: 'error',
        title: 'Delete failed',
        message: err instanceof Error ? err.message : 'Could not delete property',
      });
    } finally {
      setDeleting(false);
    }
    ...
    <div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          message={search ? 'Try a different search term or filter.' : 'Get started by adding your first property.'}
          action={
            !search && (
              <Button onClick={openCreate}>
                <Plus className="mr-2 h-4 w-4" />
            ...
            <table className="w-full min-w-[40rem] text-left text-sm">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 font-medium text-gray-700">Bedrooms</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredItems.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-gray-100">
                          {p.image ? (
                            <img
                              src={`${import.meta.env.VITE_POCKETBASE_URL}/api/files/properties/${p.id}/${p.image}`}
                              alt={p.name}
                    ...
                    <td className="px-6 py-4 text-gray-900">{p.bedrooms}</td>
