...
import { MessageSquare, PenTool, Tags, Users } from 'lucide-react';
import {
  listAllEntries, listAllCategories, listAllMessages,
  listAllUsers, type Messages,
} from '@/lib/pocketbase';
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const entries = await listAllEntries(1, 1);
        const categories = await listAllCategories();
        const messages = await listAllMessages(1, 5);
        const users = await listAllUsers(1, 1);

        setStats({
          entries: entries.totalItems,
          categories: categories.length,
          messages: messages.totalItems,
          recentRequests: messages.items,
  ...
  const statCards = [
    { label: 'Entries', value: stats.entries, icon: PenTool },
                ...
                <tbody>
                  {stats.recentRequests.map((r) => (
                    <tr key={r.id} className="border-b border-gray-100 last:border-0">
                      ...
                      <td className="py-3 text-gray-600">{r.email || r.expand?.user?.email || r.user || '-'}</td>
