...
import { CalendarCheck, Tags, Users, Utensils } from 'lucide-react';
import {
  listAllMenuItems, listAllCategories, listAllReservations,
  listAllUsers, type Reservations,
} from '@/lib/pocketbase';
...
export default function AdminDashboard() {
  ...
  const [stats, setStats] = useState({
    ...
    categories: 0,
    reservations: 0,
    recentRequests: [] as Reservations[],
    users: 0,
    loading: true
  });
  ...
  useEffect(() => {
    async function load() {
      try {
        const menu_items = await listAllMenuItems(1, 1);
        const categories = await listAllCategories();
        const reservations = await listAllReservations(1, 5);
        const users = await listAllUsers(1, 1);

        setStats({
          menu_items: menu_items.totalItems,
          categories: categories.length,
          reservations: reservations.totalItems,
          recentRequests: reservations.items,
  ...
  const statCards = [
    { label: 'Menu Items', value: stats.menu_items, icon: Utensils },
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <AdminCard className="lg:col-span-2">
          ...
          {stats.recentRequests.length === 0 ? (
            <p className="mt-2 text-sm text-gray-600">No reservations yet.</p>
          ) : (
