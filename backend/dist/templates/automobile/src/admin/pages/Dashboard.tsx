...
import { Car, MessageSquare, Tags, Users } from 'lucide-react';
import {
  listAllListings, listAllCategories, listAllInquiries,
  listAllUsers, type Inquiries,
} from '@/lib/pocketbase';
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const listings = await listAllListings(1, 1);
        const categories = await listAllCategories();
        const inquiries = await listAllInquiries(1, 5);
        ...
        setStats({
          ...
          recentRequests: inquiries.items,
  ...
  const statCards = [
    { label: 'Listings', value: stats.listings, icon: Car },
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <AdminCard className="lg:col-span-2">
          ...
          {stats.recentRequests.length === 0 ? (
            <p className="mt-2 text-sm text-gray-600">No inquiries yet.</p>
          ) : (
