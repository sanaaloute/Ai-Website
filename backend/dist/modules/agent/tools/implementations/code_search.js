...
class CodeSearchTool extends types_1.AgentTool {
    constructor() {
        ...
        this.description = `Search the codebase for code patterns, functions, components, or concepts. This is a keyword-based search (grep over up to 3 extracted keywords, scored by relevance) — it does not understand semantics, so use precise identifiers and try synonyms when a search misses.
        ...
        try {
            ...
            for (const keyword of keywords.slice(0, 3)) {
                const cmd = args.file_pattern
                    ? `grep -r -n -i -e ${(0, shell_1.shellQuote)(keyword)} --include=${(0, shell_1.shellQuote)(args.file_pattern)} --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=dist .`
                    : `grep -r -n -i -e ${(0, shell_1.shellQuote)(keyword)} --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=dist .`;
