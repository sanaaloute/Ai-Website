...
function applyFileEdit(original, editContent) {
    ...
    for (let i = 0; i < parts.length - 1; i += 2) {
        ...
        if (beforeIndex === -1) {
            throw new errors_1.DeterministicToolError(`Could not find edit anchor in original file. Looking for: "${beforeAnchor.slice(0, 80)}..."`);
        }
        if (afterAnchor) {
            let afterIndex = findInText(result, afterAnchor, beforeIndex + beforeAnchor.length);
            if (afterIndex === -1) {
                throw new errors_1.DeterministicToolError(`Could not find edit anchor in original file. Looking for: "${afterAnchor.slice(0, 80)}..."`);
            }
...
class EditFileTool extends types_1.AgentTool {
    ...
    async _call(args) {
        ...
        if (this.agentContext.fileManifest.isProtected(normalizedPath)) {
            ...
            throw new errors_1.DeterministicToolError(error);
        }
        let originalContent = "";
        try {
            ...
            if (hasSyntaxError) {
                ...
                this.agentContext.streamWriter.write((0, stream_writer_1.createFileUpdateEvent)(normalizedPath, originalContent, "modified"));
                if (this.agentContext.supabaseProjectId) {
                    try {
            ...
            if (error instanceof errors_1.DeterministicToolError)
                throw error;
