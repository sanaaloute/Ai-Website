...
async function generateMetaDescription(state, deps) {
    ...
    try {
        const raw = await deps.aiGateway.chatCompletionsStream([{ role: 'user', content: prompt }], deps.modelResolver.resolveSequence('seo_meta'), state.aiCredentials, undefined, deps.signal, deps.modelResolver.generationParams('seo_meta'));
