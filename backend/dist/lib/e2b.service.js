...
exports.isSandboxGoneError = isSandboxGoneError;
...
function isSandboxGoneError(err) {
    ...
    return (msg.includes('sandbox_not_found') ||
    ...
    async doRenewSandbox(currentId) {
        ...
        try {
            ...
            if (!isSandboxGoneError(err))
                throw err;
            this.logger.warn(`Source sandbox ${currentId} is already gone; renewing into an empty sandbox (no files to migrate)`);
            this.sandboxes.delete(currentId);
            sourceGone = true;
        ...
        try {
            ...
            this.logger.log(`Renewed sandbox ${currentId} -> ${newSandbox.sandboxId} (${filesMigrated} files${sourceGone ? ', source was already gone' : ''}, ${Date.now() - start}ms)`);
            return {
                ...newSandbox,
                filesMigrated,
                sourceGone,
