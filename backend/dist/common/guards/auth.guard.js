...
let AuthGuard = class AuthGuard {
    ...
    async canActivate(context) {
        ...
        if (error || !data.user) {
            throw new common_1.UnauthorizedException({ success: false, error: 'Invalid session' });
        }
        ...
        if (profileError || !profile) {
            throw new common_1.UnauthorizedException({ success: false, error: 'User no longer exists' });
        }
        req.user = data.user;
        return true;
    }
