...
|---|---|---|
| `status` | `{ "data": { "status": "analyzing", "message": "..." } }` | Current graph stage started |
| `todos_update` | `{ "data": { "todos": [...] } }` | Planner/validator produced a task list |
| `file_update` | `{ "data": { "path": "...", "status": "created|modified|deleted", "size": 123, "lineCount": 5 } }` | Executor wrote a file to the sandbox; fetch full content via `GET /api/get-sandbox-file` |
| `review` | `{ "data": { "passed": true, "issues": [], "suggestions": [] } }` | Reviewer result |
| `preview` | `{ "data": { "url": "https://...e2b.dev" } }` | Preview URL after finalize |
| `error` | `{ "data": { "message": "..." } }` | Fatal or step error |
| `done` | `{ "data": { "finalResponse": "...", "snapshotId": "..." } }` | Graph finished (snapshotId lets clients roll back) |

Example:
```
data: {"type":"status","data":{"status":"analyzing","message":"Analyzing your request..."}}

data: {"type":"todos_update","data":{"todos":[{"id":"1","content":"...","status":"pending"}]}}

data: {"type":"file_update","data":{"path":"src/App.tsx","status":"modified","size":1234,"lineCount":42}}

data: {"type":"preview","data":{"url":"https://5173-<sandboxId>.e2b.dev"}}

data: {"type":"done","data":{"finalResponse":"Landing page created successfully"}}
```
...
**SSE Response**
```
data: {"type":"start","message":"Applying AI code"}

data: {"type":"step","message":"Parsing response","packages":[]}

data: {"type":"file-progress","fileName":"src/App.tsx"}

data: {"type":"file-complete","fileName":"src/App.tsx"}

data: {"type":"step","message":"Installing 1 packages","packages":["framer-motion"]}

data: {"type":"complete","completionAck":{"status":"applied","token":"<uuid>","streamVersion":1},"results":{"packagesInstalled":["framer-motion"],"packagesFailed":[],"filesCreated":["src/App.tsx"],"filesUpdated":[],"commandsExecuted":["npm install framer-motion"],"errors":[]}}
```
...
Migrate files from an expiring sandbox to a new one. The new sandbox is created with a 1-hour TTL, dependencies are reinstalled, and the Vite dev server is started so the preview URL is ready immediately.
...
**SSE Response**
```
data: {"type":"start","packages":["framer-motion","lucide-react"]}

data: {"type":"status","message":"Installing 2 packages..."}

data: {"type":"success","package":"framer-motion","exitCode":0}

data: {"type":"complete","results":[],"appliedFiles":[],"analyzerDone":true}
```
...
### `GET /api/get-sandbox-pocketbase-info`

Return the PocketBase preview URL and admin credentials for a sandbox that has PocketBase enabled.
...
{
  "success": true,
  "url": "https://8090-sandbox-xxx.e2b.dev",
  "adminUrl": "https://8090-sandbox-xxx.e2b.dev/_/",
  "adminEmail": "admin@...local",
  "adminPassword": "..."
}
```

If PocketBase is not running in the sandbox, `url` and credentials are `null`.
...
Endpoints for managing the PocketBase e-commerce backend template and retrieving sandbox credentials.

### `GET /api/pocketbase/template`

Return the complete PocketBase e-commerce template: schema description, frontend SDK source, and all deployment files.
...
{
  "success": true,
  "schema": { "backend": "pocketbase", "collections": { ... } },
  "sdkSource": "import PocketBase from 'pocketbase'; ...",
  "files": [
    { "path": "docker-compose.yaml", "content": "..." },
    { "path": "pocketbase/Dockerfile", "content": "..." }
  ],
  "fileCount": 7
}
```
...
### `POST /api/pocketbase/prepare-deploy`

Render the PocketBase deployment template for a specific domain.
...
### `GET /api/pocketbase/info`

Return PocketBase URL and admin credentials for a sandbox.
