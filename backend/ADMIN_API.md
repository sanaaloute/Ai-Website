...
The access token is stored in an `httpOnly` cookie named `lc_admin_token`. The backend sets this cookie on successful login. Subsequent requests must include credentials so the browser sends the cookie automatically. For backwards compatibility, the `Authorization: Bearer <admin_access_token>` header is also accepted.

Tokens expire after 24 hours by default (configurable via `ADMIN_JWT_EXPIRY_MINUTES`).
...
Authenticate an admin. On success the backend sets the `lc_admin_token` `httpOnly` cookie and returns the admin profile.
...
Sign out the current admin and clear the `lc_admin_token` cookie.
...
All admin dashboard routes require the `lc_admin_token` `httpOnly` cookie (or `Authorization: Bearer <admin_access_token>` as a fallback).
