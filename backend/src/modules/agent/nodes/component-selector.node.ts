...
function defaultComponents(category?: string): string[] {
...
export async function componentSelectorNode(state: AgentState, deps: GraphDependencies): Promise<Partial<AgentState>> {
  ...
  const messages: ToolLoopMessage[] = [
    ...
    { role: 'user', content: promptToString(buildPromptContent(`Design spec and request: ${context}`, state.prompt)) },
  ];
  ...
  try {
    ...
    const components = Array.isArray(parsed?.componentsToInstall)
      ? (parsed?.componentsToInstall as string[])
      : [];
