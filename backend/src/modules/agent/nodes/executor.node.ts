import { AgentState, TodoItem } from '../state';
...
import { promptToString } from '@/types';

function buildExecutorContext(state: AgentState): string {
...
async function buildFilesWritten(
  manifest: FileManifest,
  sandboxProvider: SandboxProvider,
  deps: GraphDependencies,
): Promise<Array<{ path: string; status: string; content?: string; error?: string }>> {
  const changed = manifest.listChanged();
  const filesWritten: Array<{ path: string; status: string; content?: string; error?: string }> = [];
...
export async function executorNode(state: AgentState, deps: GraphDependencies): Promise<Partial<AgentState>> {
  ...
  try {
    ...
    while (iteration < maxIterations) {
      ...
      const { content, toolCalls, toolResults } = await deps.aiGateway.chatCompletionsWithToolsStream(
        ...
        async (path) => {
          ...
          // Announce the target file early so the UI header stays in sync with
    ...
    // Install any packages the planner explicitly requested
    ...
    for (const pkg of plannedPackages) {
      ...
      if (addDependencyTool) {
        try {
          resultText = await addDependencyTool.invoke({ package: pkg }) as string;
          success = true;
        } catch (err) {
