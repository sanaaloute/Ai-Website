...
export class PromptLoaderService {
  ...
  constructor() {
    ...
    // build output (dist/prompts). This file is at modules/agent/services, so
    // we walk up three levels to reach the project root and then into prompts.
    const compiledDir = path.resolve(__dirname, '..', '..', '..', 'prompts');
    this.promptsDir = existsSync(compiledDir) ? compiledDir : path.resolve(process.cwd(), 'src', 'prompts');
  }
