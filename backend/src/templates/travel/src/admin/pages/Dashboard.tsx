...
import { Map, Tags, Ticket, Users } from 'lucide-react';
import {
  listAllTours, listAllCategories, listAllBookings,
  listAllUsers, type Bookings,
} from '@/lib/pocketbase';
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const tours = await listAllTours(1, 1);
        const categories = await listAllCategories();
        const bookings = await listAllBookings(1, 5);
  ...
  const statCards = [
    { label: 'Tours', value: stats.tours, icon: Map },
