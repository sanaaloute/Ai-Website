...
const emptyTours: Partial<Tours> = {
  name: '',
  slug: '',
  destination: '',
  duration: '',
...
export default function AdminTours() {
  ...
  const [editing, setEditing] = useState<Tours | null>(null);
  const [form, setForm] = useState<Partial<Tours>>(emptyTours);
  const [saving, setSaving] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<Tours | null>(null);
  ...
  const load = async (targetPage = page) => {
    ...
    try {
      const [p, c] = await Promise.all([
        listAllTours(targetPage, PER_PAGE, { search: search || undefined }),
        listAllCategories(),
      ]);
      setItems(p.items);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const data = { ...form };
      if (editing) {
        await updateTours(editing.id, data);
      } else {
        await createTours(data);
      }
      setModalOpen(false);
      await load();
      addToast({
      ...
      addToast({
        variant: 'error',
        title: 'Save failed',
        message: err instanceof Error ? err.message : 'Could not save tour',
      });
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await deleteTours(deleteTarget.id);
      setDeleteTarget(null);
      await load();
      addToast({ variant: 'success', title: 'Deleted', message: 'Tour has been removed.' });
    } catch (err) {
      addToast({
        variant: 'error',
        title: 'Delete failed',
        message: err instanceof Error ? err.message : 'Could not delete tour',
      });
    } finally {
      setDeleting(false);
    }
    ...
    <div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          message={search ? 'Try a different search term or filter.' : 'Get started by adding your first tour.'}
          action={
            !search && (
              <Button onClick={openCreate}>
                <Plus className="mr-2 h-4 w-4" />
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-gray-100">
                          {p.image ? (
                            <img
                              src={`${import.meta.env.VITE_POCKETBASE_URL}/api/files/tours/${p.id}/${p.image}`}
                              alt={p.name}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit tour' : 'Add tour'}>
