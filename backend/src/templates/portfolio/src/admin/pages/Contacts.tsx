...
export default function AdminContacts() {
  ...
  const load = async (currentPage = page, currentSearch = search) => {
    setLoading(true);
    try {
      const r = await listAllContacts(currentPage, PER_PAGE, {
      ...
      addToast({
        ...
        message: err instanceof Error ? err.message : 'Could not fetch contacts.',
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await deleteContacts(deleteId);
      addToast({ variant: 'success', title: 'Deleted', message: 'Contact has been deleted.' });
      await load(page, search);
    } catch (err) {
      addToast({
        variant: 'error',
        title: 'Delete failed',
        message: err instanceof Error ? err.message : 'Could not delete contact.',
      });
    } finally {
      setDeleting(false);
      setDeleteId(null);
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <EmptyState
          title="No contacts"
          message={search ? 'No contacts match your search.' : 'There are no contacts yet.'}
        />
      ) : (
        <TableContainer>
