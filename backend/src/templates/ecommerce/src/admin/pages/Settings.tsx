...
export default function AdminSettings() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!settings.name?.trim() || !settings.currency?.trim()) {
      addToast({ variant: 'warning', title: 'Validation', message: 'Store name and currency are required.' });
      return;
    }
    ...
    try {
      ...
      addToast({ variant: 'success', title: 'Saved', message: 'Store settings have been updated.' });
    } catch (err) {
      addToast({
        variant: 'error',
        title: 'Save failed',
