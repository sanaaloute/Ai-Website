...
import { Shirt, ShoppingBag, Tags, Users } from 'lucide-react';
import {
  listAllProducts, listAllCategories, listAllOrders,
  listAllUsers, type Orders,
} from '@/lib/pocketbase';
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const products = await listAllProducts(1, 1);
        const categories = await listAllCategories();
        const orders = await listAllOrders(1, 5);
  ...
  const statCards = [
    { label: 'Products', value: stats.products, icon: Shirt },
