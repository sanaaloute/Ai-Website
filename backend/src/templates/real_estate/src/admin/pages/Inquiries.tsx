...
export default function AdminInquiries() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<Inquiries | null>(null);
  const [deleting, setDeleting] = useState(false);

  const load = async (targetPage = page) => {
    setLoading(true);
    try {
      const r = await listAllInquiries(targetPage, PER_PAGE, {
  ...
  const handleStatusChange = async (id: string, status: Inquiries['status']) => {
    try {
      ...
      await load();
      addToast({ variant: 'success', title: 'Status updated', message: 'Inquiry status has been updated.' });
    } catch (err) {
      addToast({
        variant: 'error',
        title: 'Update failed',
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await load();
      addToast({ variant: 'success', title: 'Deleted', message: 'Inquiry has been removed.' });
    } catch (err) {
      addToast({
        variant: 'error',
        title: 'Delete failed',
        message: err instanceof Error ? err.message : 'Could not delete inquiry',
      });
    } finally {
      setDeleting(false);
    }
    ...
    <div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          message={search || statusFilter !== 'all' ? 'Try a different search term or filter.' : 'No inquiries have been submitted yet.'}
        />
      ) : (
            ...
            <table className="w-full min-w-[40rem] text-left text-sm">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 font-medium text-gray-700">Property</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map((r) => (
                  <tr key={r.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-gray-600">{r.expand?.property?.name || r.property || '-'}</td>
      ...
      <ConfirmDialog
        ...
        message={`Are you sure you want to delete inquiry #${deleteTarget?.id.slice(-6)}? This action cannot be undone.`}
        confirmText="Delete"
