...
const emptyListings: Partial<Listings> = {
  name: '',
  slug: '',
  make: '',
  model: '',
...
export default function AdminListings() {
  ...
  const [editing, setEditing] = useState<Listings | null>(null);
  const [form, setForm] = useState<Partial<Listings>>(emptyListings);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [p, c] = await Promise.all([
        listAllListings(page, PER_PAGE, { search: search.trim() || undefined }),
        listAllCategories(),
      ]);
      setItems(p.items);
      ...
      addToast({ variant: 'error', title: 'Failed to load listings', message });
    } finally {
      setLoading(false);
    }
  ...
  const openCreate = () => {
    setEditing(null);
    setForm(emptyListings);
    setModalOpen(true);
  };

  const openEdit = (item: Listings) => {
    setEditing(item);
    setForm({ ...item });
    setModalOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const data = { ...form };
      if (editing) {
        await updateListings(editing.id, data);
        addToast({ variant: 'success', title: 'Saved', message: 'Listing updated successfully.' });
      } else {
        await createListings(data);
        addToast({ variant: 'success', title: 'Created', message: 'Listing created successfully.' });
      }
      setModalOpen(false);
      await load();
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await deleteListings(deleteId);
      addToast({ variant: 'success', title: 'Deleted', message: 'Listing deleted successfully.' });
      await load();
    } catch (err) {
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <EmptyState
          title="No listings"
          message={search ? 'No listings match your search.' : 'Get started by adding your first listing.'}
          action={
            <Button onClick={openCreate}>
              <Plus className="mr-2 h-4 w-4" />
              Add listing
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 font-medium text-gray-700">Year</th>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit listing' : 'Add listing'}>
