...
import { BookOpen, Tags, UserPlus, Users } from 'lucide-react';
import {
  listAllCourses, listAllCategories, listAllEnrollments,
  listAllUsers, type Enrollments,
} from '@/lib/pocketbase';
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const courses = await listAllCourses(1, 1);
        const categories = await listAllCategories();
        const enrollments = await listAllEnrollments(1, 5);
        ...
        setStats({
          ...
          recentRequests: enrollments.items,
  ...
  const statCards = [
    ...
    { label: 'Enrollments', value: stats.enrollments, icon: UserPlus },
