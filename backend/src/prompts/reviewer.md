...
- **Preview handshake is present.** Verify that `src/main.tsx` posts `LOVECODE_PREVIEW_READY` to `window.parent` after mounting, and that `index.html` includes the blank-page safety script. If either is missing, flag it as critical.
