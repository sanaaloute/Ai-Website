...
import { ColorPicker } from '@/components/visual-editing/ColorPicker';
import { NumberInput } from '@/components/visual-editing/NumberInput';
import { StylePopover } from '@/components/visual-editing/StylePopover';
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
  TooltipTrigger
} from '@/components/ui/shadcn/tooltip-radix';
import { Label } from '@/components/ui/shadcn/label';
import type { ComponentSelection } from '@/lib/visual-editing/types';
import { rgbToHex, processNumericValue } from '@/lib/visual-editing/style-utils';
import {
...
export function VisualEditingToolbar({
  ...
  const sendStyleModification = (styles: {
    ...
    text?: { fontSize?: string; fontWeight?: string; color?: string; fontFamily?: string };
  }) => {
    ...
    setPendingChanges((prev) => {
      ...
      const newStyles: Record<string, unknown> = { ...existing?.styles };
      ...
      updated.set(selectedComponent.id, {
        ...
        styles: newStyles as import('@/lib/visual-editing/types').VisualEditingChange['styles'],
  ...
  }, [selectedComponent, getCurrentElementStyles]);
  ...
  return (
    <TooltipProvider delayDuration={300}>
      <div
        className="border-border bg-background absolute z-50 flex flex-row items-center gap-1 rounded-md border p-2 shadow-lg"
        style={{
          top: `${toolbarTop}px`,
          left: `${toolbarLeft}px`
        }}
      >
        ...
        {isDynamic ? (
          <div className="flex items-center rounded px-2 py-1 text-xs font-medium text-yellow-800 dark:text-yellow-200">
              ...
              <StylePopover icon={<Type size={16} />} title="Text Style" tooltip="Text Style">
                <div className="space-y-2">
                  ...
                  <div>
                    ...
                    <select
                      id="font-weight"
                      className="border-input bg-background mt-1 h-8 w-full rounded-md border px-3 py-2 text-xs"
