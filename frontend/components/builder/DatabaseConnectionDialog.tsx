"use client";

import { useState, useEffect } from "react";
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogFooter,
  DialogHeader,
  DialogTitle
} from "@/components/ui/shadcn/dialog";
import { cn } from "@/utils/cn";
import { panelClass, DialogSurfaceDecor } from "./DialogSurfaceDecor";

export type DatabaseConnectionValue = {
  type: "supabase" | "mongodb";
  supabaseUrl?: string;
  supabaseAnonKey?: string;
  mongoUri?: string;
};

type DatabaseConnectionDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  value: DatabaseConnectionValue | null;
  onSave: (v: DatabaseConnectionValue) => void;
};

export function DatabaseConnectionDialog({
  open,
  onOpenChange,
  value,
  onSave
}: DatabaseConnectionDialogProps) {
  const [kind, setKind] = useState<"supabase" | "mongodb">("supabase");
  const [supabaseUrl, setSupabaseUrl] = useState("");
  const [supabaseAnonKey, setSupabaseAnonKey] = useState("");
  const [mongoUri, setMongoUri] = useState("");

  useEffect(() => {
    if (!open) return;
    const timer = window.setTimeout(() => {
      if (value?.type === "mongodb") {
        setKind("mongodb");
        setMongoUri(value.mongoUri ?? "");
        setSupabaseUrl("");
        setSupabaseAnonKey("");
      } else {
        setKind("supabase");
        setSupabaseUrl(value?.supabaseUrl ?? "");
        setSupabaseAnonKey(value?.supabaseAnonKey ?? "");
        setMongoUri("");
      }
    }, 0);
    return () => window.clearTimeout(timer);
  }, [open, value]);

  const handleSave = () => {
    if (kind === "supabase") {
      onSave({
        type: "supabase",
        supabaseUrl: supabaseUrl.trim(),
        supabaseAnonKey: supabaseAnonKey.trim()
      });
    } else {
      onSave({
        type: "mongodb",
        mongoUri: mongoUri.trim()
      });
    }
    onOpenChange(false);
  };

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={cn(panelClass, "max-w-md gap-0 p-0")}>
        <DialogSurfaceDecor variant="database" />
        <div className="relative z-[1] space-y-4 p-6">
        <DialogHeader className="space-y-2 text-left">
          <DialogTitle className="text-xl font-semibold tracking-tight text-zinc-50">
            Connect to database
          </DialogTitle>
          <DialogDescription className="text-zinc-400">
            Supabase or MongoDB details are added to AI context for this session so generated code can use your
            backend. Treat keys as sensitive — only use projects you own.
          </DialogDescription>
        </DialogHeader>

        <div className="flex gap-2 rounded-xl border border-white/10 bg-white/[0.03] p-1">
          {(["supabase", "mongodb"] as const).map((k) => (
            <button
              key={k}
              type="button"
              onClick={() => setKind(k)}
              className={cn(
                "flex-1 rounded-lg px-3 py-2 text-xs font-medium transition",
                kind === k
                  ? "bg-gradient-to-r from-primary/50 to-primary-soft/40 text-white shadow-[0_0_20px_rgba(124,58,237,0.2)]"
                  : "text-zinc-500 hover:bg-white/5 hover:text-zinc-300"
              )}
            >
              {k === "supabase" ? "Supabase" : "MongoDB"}
            </button>
          ))}
        </div>

        {kind === "supabase" ? (
          <div className="space-y-3">
            <label className="block text-xs font-medium text-zinc-400">
              Project URL
              <input
                value={supabaseUrl}
                onChange={(e) => setSupabaseUrl(e.target.value)}
                placeholder="https://xxxx.supabase.co"
                className="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-zinc-100 outline-none placeholder:text-zinc-600 focus:border-glow-cyan/50"
                autoComplete="off"
              />
            </label>
            <label className="block text-xs font-medium text-zinc-400">
              Anon (public) key
              <input
                value={supabaseAnonKey}
                onChange={(e) => setSupabaseAnonKey(e.target.value)}
                placeholder="eyJ..."
                type="password"
                className="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-zinc-100 outline-none placeholder:text-zinc-600 focus:border-glow-cyan/50"
                autoComplete="off"
              />
            </label>
          </div>
        ) : (
          <label className="block text-xs font-medium text-zinc-400">
            Connection string
            <input
              value={mongoUri}
              onChange={(e) => setMongoUri(e.target.value)}
              placeholder="mongodb+srv://..."
              type="password"
              className="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-zinc-100 outline-none placeholder:text-zinc-600 focus:border-glow-cyan/50"
              autoComplete="off"
            />
          </label>
        )}

        <DialogFooter className="gap-2 sm:gap-2">
          <button
            type="button"
            onClick={() => onOpenChange(false)}
            className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-zinc-300 transition hover:bg-white/10"
          >
            Cancel
          </button>
          <button
            type="button"
            onClick={handleSave}
            className="rounded-xl bg-gradient-to-r from-primary to-primary-soft px-4 py-2 text-sm font-semibold text-white shadow-soft-glow transition hover:opacity-95"
          >
            Save for this session
          </button>
        </DialogFooter>
        </div>
      </DialogContent>
    </Dialog>
  );
}
