...
function GenerationPreviewPanelComponent({
    ...
    return (
      <div className="relative w-full h-full bg-zinc-950">
        {urlScreenshot && (
          ...
          <img
            src={urlScreenshot}
            alt={t('preview.websitePreviewAlt')}
        ...
        {shouldShowLoadingOverlay && (
          <GenerationPreviewWait
            line1={
              isCapturingScreenshot
                ? t('preview.analyzingWebsite')
                : isPreparingDesign
                  ? t('preview.preparingDesign')
                  : generationProgress.isGenerating
                    ? t('preview.generatingApp')
                    : t('preview.loading')
            }
            line2={
              isCapturingScreenshot
                ? t('preview.capturingLayout')
                : isPreparingDesign
                  ? t('preview.understandingLayout')
                  : generationProgress.isGenerating
                    ? t('preview.streamingComponents')
                    : t('preview.pleaseWait')
            }
          />
        )}
      </div>
  ...
  if (sandboxData?.url && isBuilding) {
    return (
      <div className="relative w-full h-full bg-zinc-950">
        <GenerationPreviewWait
          line1={t('preview.buildingWebsite')}
          line2={t('preview.buildingHint')}
        />
      </div>
    );
  }
  ...
  if (sandboxData?.url && !hasBuiltContent) {
    return (
      <div className="flex h-full w-full flex-col items-center justify-center bg-zinc-950 px-6 text-center">
        ...
        <h3 className="text-sm font-medium text-zinc-300">{t('preview.placeholderTitle')}</h3>
        <p className="mt-1 max-w-sm text-xs text-zinc-600">
          {t('preview.placeholderHint')}
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="relative w-full h-full">
            ...
            <div className="w-full max-w-2xl overflow-hidden rounded-2xl border border-red-500/30 bg-black/95 p-4 shadow-[0_20px_50px_rgba(0,0,0,0.55)] backdrop-blur-xl">
              ...
              <div className="mt-3 flex items-center justify-end gap-2">
                <button
                  ...
                  aria-label={t('preview.copyErrorAria')}
                >
                  {t('preview.copyError')}
                </button>
                <button
                  ...
                  className="rounded-lg border border-white/[0.06] bg-white/[0.03] px-2.5 py-1 text-xs text-zinc-400 transition hover:bg-white/[0.06] hover:text-zinc-200"
                  aria-label={t('preview.dismissErrorAria')}
                >
                  {t('preview.dismiss')}
                </button>
                <button
        ...
        <button
          ...
          title={t('preview.refreshSandboxTitle')}
        >
    ...
    <div className="flex items-center justify-center h-full bg-zinc-950 text-zinc-500">
      {screenshotError ? (
        <div className="text-center space-y-2">
          <p className="mb-1 text-sm text-zinc-400">{t('preview.screenshotFailed')}</p>
        ...
        <div className="w-full px-4 py-8">
          <GenerationCognitionLoader
            ...
            title={t('preview.loadingPreviewTitle')}
            subtitle={t('preview.loadingPreviewSubtitle')}
            autoProgress
        ...
        <div className="text-center space-y-2">
          <p className="text-xs text-zinc-600">{t('preview.startChatting')}</p>
          <button
            type="button"
            onClick={() => void reloadPreview()}
            className="inline-flex items-center rounded-lg border border-white/[0.08] bg-white/[0.03] px-3 py-1.5 text-xs font-medium text-zinc-400 transition hover:bg-white/[0.06] hover:text-zinc-200"
