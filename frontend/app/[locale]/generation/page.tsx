...
function AISandboxPage() {
  ...
  const headerWorkspace = useMemo(
    () => ({
      ...
      openGitccPushDialog: integrations.openGitccPushDialog,
      integrationReadiness: cloudActions.integrationReadiness,
      integrationBusy: ui.integrationBusy,
      hostOnOpenHost: integrations.hostOnOpenHost,
    }),
    [
      sandbox.sandboxData,
      router,
      ...
      ui.isDownloadingZip,
      integrations.openGitccPushDialog,
      cloudActions.integrationReadiness,
      ui.integrationBusy,
      integrations.hostOnOpenHost,
    ]
  );
  ...
  const currentProjectOpenHostInfo = useMemo(() => {
    const current = cloud.cloudProjects.find(
      (p) => p.projectId === ui.currentSessionProjectId
    );
    return current
      ? {
          appUuid: current.openhostAppUuid,
          domainUrl: current.openhostDomainUrl,
          pocketbaseAdminUrl: current.pocketbaseAdminUrl,
        }
      : null;
  ...
  const dialogOverlaysWorkspace = useMemo(
    () => ({
      ...
      gitccPushDialogOpen: ui.gitccPushDialogOpen,
      setGitccPushDialogOpen: ui.setGitccPushDialogOpen,
      gitccPushResult: ui.gitccPushResult,
      setGitccPushResult: ui.setGitccPushResult,
      defaultGitccRepoName: integrations.defaultGitccRepoName,
      executeGitccPush: integrations.executeGitccPush,
      executeOpenHostDeploy: integrations.executeOpenHostDeploy,
      integrationBusy: ui.integrationBusy,
      openHostDeployDialogOpen: ui.openHostDeployDialogOpen,
      setOpenHostDeployDialogOpen: ui.setOpenHostDeployDialogOpen,
      openHostDeployResult: ui.openHostDeployResult,
      setOpenHostDeployResult: ui.setOpenHostDeployResult,
      openHostDeployCard: ui.openHostDeployCard,
      setOpenHostDeployCard: ui.setOpenHostDeployCard,
      currentSessionProjectId: ui.currentSessionProjectId,
    ...
    [
      ...
      ui.gitccPushDialogOpen,
      ui.setGitccPushDialogOpen,
      ui.gitccPushResult,
      ui.setGitccPushResult,
      integrations.defaultGitccRepoName,
      integrations.executeGitccPush,
      integrations.executeOpenHostDeploy,
      ui.integrationBusy,
      ui.openHostDeployDialogOpen,
      ui.setOpenHostDeployDialogOpen,
      ui.openHostDeployResult,
      ui.setOpenHostDeployResult,
      ui.openHostDeployCard,
      ui.setOpenHostDeployCard,
      ui.currentSessionProjectId,
      currentProjectOpenHostInfo,
